...
//Midpoint of Linked List by runners technique
...
//kth node from Last of the linked List
...
node* mergeTwoSortedLinkedList(node*a,node*b){
    ...
    if(a->data<b->data){
        c=a;
        c->next=mergeTwoSortedLinkedList(a->next,b);
    }
    else{
        c=b;
        c->next=mergeTwoSortedLinkedList(a,b->next);
    }
    return c;
}
...
int main() {
	...
	node*k=kThFromLast(head,3);
	cout<<"The Kth node from Last is : "<<k->data<<endl;*/
	cout<<endl;
